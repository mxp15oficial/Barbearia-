// Lista de cortes simulados no banco de dados local
const listaCortes = [
  { id: 1, nome: "Degradê Navalhado", preco: 35.00, img: "https://picsum.photos/300/300?random=1" },
  { id: 2, nome: "Corte Social VIP", preco: 30.00, img: "https://picsum.photos/300/300?random=2" },
...
// Cadastro de usuário com DDI (55)
...
// Carregar a grade de quadrados com as fotos de cabelos
...
// Abrir Modal do Corte selecionado
...
// Confirmar Corte (Botão ✅ -> Abre Tela de Agendamento)
...
// Finalizar Agendamento com Varredura de Conflito (< 30 min)
document.getElementById('btnFinalizarAgendamento').addEventListener('click', () => {
  ...
  // Varredura para verificar conflito de até 30 minutos de diferença
  const conflito = agendamentosExistentes.some(item => {
    if (item.data === data) {
      ...
      return diferenca < 30; // Se a diferença for menor que 30 min, bloqueia
    }
    return false;
  });
  ...
  // Registra o agendamento caso o horário esteja livre
