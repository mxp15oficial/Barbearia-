...
// Verifica se a pessoa já visitou/cadastrou antes
window.addEventListener('DOMContentLoaded', () => {
  const jaVisitou = localStorage.getItem('usuario_ja_cadastrado');

  if (jaVisitou) {
    // Esconde o botão de cadastro na 2ª visita em diante
    document.getElementById('tabCadastro').classList.add('hidden');
    document.getElementById('tabLogin').click();
  } else {
    // Na 1ª visita, abre direto na aba de cadastro
...
// CADASTRO PRIMEIRA VEZ: Entra direto na plataforma sem pedir login
document.getElementById('formCadastro').addEventListener('submit', (e) => {
  ...
  // Salva dados do usuário e marca que ele já realizou o 1º cadastro
